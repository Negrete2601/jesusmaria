  <!-- Header Start -->
  <div class="container-fluid bg-breadcrumb2">
            <div class="container text-center py-5" style="max-width: 900px;">
                <h3 class="text-white display-3 mb-4 wow fadeInDown" data-wow-delay="0.1s">Verificadores</h1>
                <ol class="breadcrumb justify-content-center text-white mb-0 wow fadeInDown" data-wow-delay="0.3s">
                    <li class="breadcrumb-item"><a href="index.php" class="text-white">Inicio</a></li>
                    <li class="breadcrumb-item"><a href="#" class="text-white">¡Mi orgullo, mi gente!</a></li>
                    <li class="breadcrumb-item active text-secondary">Verificadores</li>
                </ol>    
            </div>
        </div>
        <!-- Header End -->

       

        

        <!-- Countries We Offer Start -->
        <div class="container-fluid country overflow-hidden py-5">
            <div class="container py-5">
                <div class="section-title text-center wow fadeInUp" data-wow-delay="0.1s" style="margin-bottom: 70px;">
                <div style="text-align:justify">
                <div id="main_content"><div class="container">
	
    <style>
        /* Estilos de la tabla */
        table {
          border-collapse: collapse;
          width: 100%;
        }
    
        th, td {
          border: 1px solid #dddddd;
          text-align: left;
          padding: 8px;
        }
    
        th {
          background-color: #f2f2f2;
        }
    
        .photo-cell img {
          max-width: 100px;
        }
      </style>
    <br>
    <div>
        <table>
          <thead>
            <tr>
              <th>Fotografía</th>
              <th>Nombre</th>
              <th>Dependencia</th>
              <th>Departamento</th>
              <th>Número de carta credencial</th>
              <th>Domicilio</th>
              <th>Facultades y/o Funciones</th>
              <th>Número de teléfono y correo electrónico de la dependencia</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td class="photo-cell"><img src="./img/1.png" alt="Ernesto Martínez Esquivel"></td>
              <td>Ernesto Martínez Esquivel</td>
              <td>Secretaria del H. Ayuntamiento</td>
              <td>Protección Civil</td>
              <td>Folio Gafete 008</td>
              <td>Calle Abasolo, esquina calle Beltrán 315, colonia centro</td>
              <td>Cumplimentar las Inspecciones y Verificaciones que se le son consignadas y Notificador.</td>
              <td>4499639921 <br> proteccioncivl_jesusmaria@outlook.com
    </td>
            </tr>
            <tr>
              <td class="photo-cell"><img src="./img/2.png" alt="Foto de Laura Guadalupe Martin Ruvalcaba"></td>
              <td>Laura Guadalupe Martin Ruvalcaba</td>
              <td>Secretaria del H. Ayuntamiento</td>
              <td>Protección Civil</td>
              <td>Folio Gafete 003</td>
              <td>Calle Abasolo, esquina calle Beltrán 315, colonia centro</td>
              <td>Inspección y Verificación, agendar las inspecciones y verificaciones de acuerdo al tiempo de vencimiento, realizar los vistos buenos de la revisión de las medidas físicas de los inmuebles inspeccionados y/o verificados, generar las ordenes de inspección y verificación.</td>
              <td>4499639921<br>proteccioncivl_jesusmaria@outlook.com</td>
            </tr>
            <tr>
              <td class="photo-cell"><img src="./img/3.png" alt="Foto de Erika Susana Ledezma Moran"></td>
              <td>Erika Susana Ledezma Moran</td>
              <td>Secretaria del H. Ayuntamiento</td>
              <td>Protección Civil</td>
              <td>Folio Gafete 002</td>
              <td>Calle Abasolo, esquina calle Beltrán 315, colonia centro</td>
              <td>Inspección y Verificación, revisión de programas internos de protección civil, planes de emergencia, elaboración de documentos favorables y no favorables en la revisión de la documentación revisada del usuario.</td>
              <td>4499639921<br>proteccioncivl_jesusmaria@outlook.com</td>
            </tr>
    
            <tr>
              <td class="photo-cell"><img src="./img/4.png" alt="Foto de Humberto Ramón Esparza"></td>
              <td>Humberto Ramón Esparza</td>
              <td>Secretaria del H. Ayuntamiento</td>
              <td>Protección Civil</td>
              <td>Folio Gafete 005</td>
              <td>Calle Abasolo, esquina calle Beltrán 315, colonia centro</td>
              <td>Inspección y Verificación, las funciones que desarrolla en este momento son las de atención a reportes de emergencia, asignado al área operativa.</td>
              <td>4499639921<br>proteccioncivl_jesusmaria@outlook.com</td>
            </tr>
    
            <tr>
              <td class="photo-cell"><img src="./img/5.png" alt="Foto de Julio Cesar Vargas Rosado"></td>
              <td>Julio Cesar Vargas Rosado</td>
              <td>Secretaria del H. Ayuntamiento</td>
              <td>Protección Civil</td>
              <td>Folio Gafete 010</td>
              <td>Calle Abasolo, esquina calle Beltrán 315, colonia centro</td>
              <td>Inspección y Verificación, las funciones que desarrolla en este momento son las de atención a reportes de emergencia, asignado al área operativa.</td>
              <td>4499639921<br>proteccioncivl_jesusmaria@outlook.com</td>
            </tr>
    
            <tr>
              <td class="photo-cell"><img src="./img/6.png" alt="Foto de Blanca Araceli Estrada Ramírez"></td>
              <td>Blanca Araceli Estrada Ramírez</td>
              <td>Secretaria del H. Ayuntamiento</td>
              <td>Protección Civil</td>
              <td>Folio Gafete 012</td>
              <td>Calle Abasolo, esquina calle Beltrán 315, colonia centro</td>
              <td>Inspección y Verificación, las funciones que desarrolla en este momento son las de atención a reportes de emergencia, asignado al área operativa.</td>
              <td>4499639921<br>proteccioncivl_jesusmaria@outlook.com</td>
            </tr>
    
            <tr>
              <td class="photo-cell"><img src="./img/7.png" alt="Foto de Luis Enrique Padilla Tapia"></td>
              <td>Luis Enrique Padilla Tapia</td>
              <td>Secretaria del H. Ayuntamiento</td>
              <td>Protección Civil</td>
              <td>Folio Gafete 011</td>
              <td>Calle Abasolo, esquina calle Beltrán 315, colonia centro</td>
              <td>Inspección y Verificación, las funciones que desarrolla en este momento son las de atención a reportes de emergencia, asignado al área operativa.</td>
              <td>4499639921<br>proteccioncivl_jesusmaria@outlook.com</td>
            </tr>
    
            <tr>
              <td class="photo-cell"><img src="./img/8.png" alt="Foto de Jaqueline del Rosario Martínez Esquivel"></td>
              <td>Jaqueline del Rosario Martínez Esquivel</td>
              <td>Secretaria del H. Ayuntamiento</td>
              <td>Protección Civil</td>
              <td>Folio Gafete 016</td>
              <td>Calle Abasolo, esquina calle Beltrán 315, colonia centro</td>
              <td>Inspección y Verificación, las funciones que desarrolla en este momento son las de atención a reportes de emergencia, asignado al área operativa.</td>
              <td>4499639921<br>proteccioncivl_jesusmaria@outlook.com</td>
            </tr>
    
            <tr>
              <td class="photo-cell"><img src="./img/9.png" alt="Foto de Gabriel Jauregui Manriquez"></td>
              <td>Gabriel Jauregui Manriquez</td>
              <td>Secretaria del H. Ayuntamiento</td>
              <td>Protección Civil</td>
              <td>Folio Gafete 013</td>
              <td>Calle Abasolo, esquina calle Beltrán 315, colonia centro</td>
              <td>Inspección y Verificación, las funciones que desarrolla en este momento son las de atención a reportes de emergencia, asignado al área operativa.</td>
              <td>4499639921<br>proteccioncivl_jesusmaria@outlook.com</td>
            </tr>
    
            <tr>
              <td class="photo-cell"><img src="./img/10.png" alt="Foto de María Guadalupe Ledesma Herrera"></td>
              <td>María Guadalupe Ledesma Herrera</td>
              <td>Secretaria del H. Ayuntamiento</td>
              <td>Protección Civil</td>
              <td>Folio Gafete 014</td>
              <td>Calle Abasolo, esquina calle Beltrán 315, colonia centro</td>
              <td>Inspección y Verificación, las funciones que desarrolla en este momento son las de atención a reportes de emergencia, asignado al área operativa.</td>
              <td>4499639921<br>proteccioncivl_jesusmaria@outlook.com</td>
            </tr>
          </tbody>
        </table>
        <br>
    </div></div></div>
		
            	</div>
                </div>
                
            </div>
        </div>
        <!-- Countries We Offer End -->